...
public class CheckLanguage extends CustomJavaAction<IMendixObject>
{
	...
	public IMendixObject executeAction() throws Exception
	{
		...
		if( this.LocaleStr != null && !"".equals(this.LocaleStr.trim()) ) {
			...
			List<IMendixObject> languageList = Core.retrieveXPathQuery(context, "//" + Language.entityName, Integer.MAX_VALUE, 0, sortMap);
			...
			for( IMendixObject lang : languageList ) {
				...
				if( this.LocaleStr.equals(langStr) ) {
					Core.getLogger(this.toString()).info("Found locale match for users: " + this.getContext().getSession().getUser(context).getName() + " - Locale [Browser:" + this.LocaleStr + "] [Mx:" + langStr + "]");
					fullMatch = lang;
				}
				else if( sameLanguage != null && langStr.startsWith( langCode ) ) {
					Core.getLogger(this.toString()).info("Found language match for users: " + this.getContext().getSession().getUser(context).getName() + " - Locale [Browser:" + this.LocaleStr + "] [Mx:" + langStr + "]");
					sameLanguage = lang;
				}
